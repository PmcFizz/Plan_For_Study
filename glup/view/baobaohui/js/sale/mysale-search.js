...
require(['jquery', 'jAlert','weixinPayJS'], function($) {
	...
	//点击搜素按钮查询订单
	document.querySelector("#searcheven").addEventListener("click", querySaleData)
		//查询全部销售数据
	function querySaleData() {
		$(".loadingIcon").show();
		$.ajax({
			type: "post",
			url: basePath + "order/queryorderlist.do",
			async: true,
			data: {
				pageIndex: pageIndex,
				pageSize: pageSize,
	...
	function setNoSaleData() {
		if (pageIndex == 1) {
			...
			str = str + '		<i class="order-null" style=""></i>';
			str = str + '	</p>';
			str = str + '	<p><span class="nullword">暂无数据</span></p>';
			str = str + '</div>';
